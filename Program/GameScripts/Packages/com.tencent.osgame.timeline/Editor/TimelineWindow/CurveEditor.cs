...
namespace TimelineEditor
{
    ...
    {
        UnityEditor.TimeArea.TimeFormat timeFormat { get; }
    ...
    [System.Serializable]
    internal class CurveEditor : UnityEditor.TimeArea, CurveUpdater
    {
        ...
        public UnityEditor.TimeArea.TimeFormat timeFormat
        {
            get
            {
                if (state != null)
                ...
                return UnityEditor.TimeArea.TimeFormat.None;
            }
        }
        ...
        internal void AddSelection(TimelineEditor.CurveSelection curveSelection)
        {
        ...
        public CurveEditor(Rect rect, CurveWrapper[] curves, bool minimalGUI) : base(minimalGUI)
        {
            ...
            hTicks = new UnityEditor.TickHandler();
            hTicks.SetTickModulos(modulos);
            vTicks = new UnityEditor.TickHandler();
        ...
        void DrawCurvesOverlay()
        {
            ...
            if (m_DraggingKey != null && settings.rectangleToolFlags == TimelineEditor.CurveEditorSettings.RectangleToolFlags.NoRectangleTool)
            {
                GUI.color = Color.white;
                ...
                for (int i = 0; i < keys.Length; ++i)
                {
                    ...
                    if (selectionIdx < selected.Count && selected[selectionIdx].key == pointIdx && selected[selectionIdx].curveID == cw.id)
                    {
                        if (selected[selectionIdx].semiSelected)
                            ...
                            DrawPoint(DrawingToViewTransformPoint(new Vector2(k.time, k.value)), CurveWrapper.SelectionMode.Selected, settings.rectangleToolFlags == TimelineEditor.CurveEditorSettings.RectangleToolFlags.NoRectangleTool ? MouseCursor.MoveArrow : MouseCursor.Arrow);
                        selectionIdx++;
                    }
                    else
                        DrawPoint(DrawingToViewTransformPoint(new Vector2(k.time, k.value)), CurveWrapper.SelectionMode.None);
