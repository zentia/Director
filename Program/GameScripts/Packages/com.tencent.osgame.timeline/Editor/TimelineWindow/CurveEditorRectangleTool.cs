...
namespace TimelineEditor
{
    internal class CurveEditorRectangleTool : RectangleTool
    {
        ...
        private TimelineEditor.CurveEditor m_CurveEditor;
        ...
        public override void Initialize(UnityEditor.TimeArea timeArea)
        {
            ...
            if (m_HBarLeft == null)
            {
                ...
                m_HBarLeft.onStartDrag += (TimelineWindowManipulator manipulator, Event evt) =>
                {
                ...
                m_HBarLeft.onDrag += (TimelineWindowManipulator manipulator, Event evt) =>
                {
                    OnScaleTime(PixelToTime(evt.mousePosition.x, frameRate));
                    return true;
                };
                m_HBarLeft.onEndDrag += (TimelineWindowManipulator manipulator, Event evt) =>
                {
                    OnEndScale();
                    return true;
                };
            ...
            if (m_HBarRight == null)
            {
                ...
                m_HBarRight.onStartDrag += (TimelineWindowManipulator manipulator, Event evt) =>
                {
                    if (hasSelection && manipulator.rect.Contains(evt.mousePosition))
                    {
                ...
                m_HBarRight.onDrag += (TimelineWindowManipulator manipulator, Event evt) =>
                {
                    OnScaleTime(PixelToTime(evt.mousePosition.x, frameRate));
                    return true;
                };
                m_HBarRight.onEndDrag += (TimelineWindowManipulator manipulator, Event evt) =>
                {
                    OnEndScale();
                    return true;
                };
            ...
            if (m_HBar == null)
            {
                ...
                m_HBar.onStartDrag += (TimelineWindowManipulator manipulator, Event evt) =>
                {
                    if (hasSelection && manipulator.rect.Contains(evt.mousePosition))
                    {
                ...
                m_HBar.onDrag += (TimelineWindowManipulator manipulator, Event evt) =>
                {
                    OnMove(new Vector2(PixelToTime(evt.mousePosition.x, frameRate), 0f));
                    return true;
                };
                m_HBar.onEndDrag += (TimelineWindowManipulator manipulator, Event evt) =>
                {
                    OnEndMove();
                    return true;
                };
            ...
            if (m_VBarBottom == null)
            {
                ...
                m_VBarBottom.onStartDrag += (TimelineWindowManipulator manipulator, Event evt) =>
                {
                    if (hasSelection && manipulator.rect.Contains(evt.mousePosition))
                    {
                ...
                m_VBarBottom.onDrag += (TimelineWindowManipulator manipulator, Event evt) =>
                {
                    OnScaleValue(PixelToValue(evt.mousePosition.y));
                    return true;
                };
                m_VBarBottom.onEndDrag += (TimelineWindowManipulator manipulator, Event evt) =>
                {
                    OnEndScale();
                    return true;
                };
            ...
            if (m_VBarTop == null)
            {
                ...
                m_VBarTop.onStartDrag += (TimelineWindowManipulator manipulator, Event evt) =>
                {
                    if (hasSelection && manipulator.rect.Contains(evt.mousePosition))
                    {
                ...
                m_VBarTop.onDrag += (TimelineWindowManipulator manipulator, Event evt) =>
                {
                    OnScaleValue(PixelToValue(evt.mousePosition.y));
                    return true;
                };
                m_VBarTop.onEndDrag += (TimelineWindowManipulator manipulator, Event evt) =>
                {
                    OnEndScale();
                    return true;
                };
            ...
            if (m_VBar == null)
            {
                ...
                m_VBar.onStartDrag += (TimelineWindowManipulator manipulator, Event evt) =>
                {
                    if (hasSelection && manipulator.rect.Contains(evt.mousePosition))
                    {
                ...
                m_VBar.onDrag += (TimelineWindowManipulator manipulator, Event evt) =>
                {
                    OnMove(new Vector2(0f, PixelToValue(evt.mousePosition.y)));
                    return true;
                };
                m_VBar.onEndDrag += (TimelineWindowManipulator manipulator, Event evt) =>
                {
                    OnEndMove();
                    return true;
                };
            ...
            if (m_SelectionBox == null)
            {
                ...
                m_SelectionBox.onStartDrag += (TimelineWindowManipulator manipulator, Event evt) =>
                {
                ...
                m_SelectionBox.onDrag += (TimelineWindowManipulator manipulator, Event evt) =>
                {
                ...
                m_SelectionBox.onEndDrag += (TimelineWindowManipulator manipulator, Event evt) =>
                {
                    OnEndMove();
                    return true;
                };
            ...
            if (m_SelectionScaleBottom == null)
            {
                ...
                m_SelectionScaleBottom.onStartDrag += (TimelineWindowManipulator manipulator, Event evt) =>
                {
                    if (hasSelection && manipulator.rect.Contains(evt.mousePosition))
                    {
                        OnStartScale(ToolCoord.Top, ToolCoord.Bottom, new Vector2(0f, PixelToValue(evt.mousePosition.y)), DragMode.ScaleVertical, false);
                ...
                m_SelectionScaleBottom.onDrag += (TimelineWindowManipulator manipulator, Event evt) =>
                {
                    OnScaleValue(PixelToValue(evt.mousePosition.y));
                    return true;
                };
                m_SelectionScaleBottom.onEndDrag += (TimelineWindowManipulator manipulator, Event evt) =>
                {
                    OnEndScale();
                    return true;
                };
            ...
            if (m_SelectionScaleTop == null)
            {
                ...
                m_SelectionScaleTop.onStartDrag += (TimelineWindowManipulator manipulator, Event evt) =>
                {
                    if (hasSelection && manipulator.rect.Contains(evt.mousePosition))
                    {
                        OnStartScale(ToolCoord.Bottom, ToolCoord.Top, new Vector2(0f, PixelToValue(evt.mousePosition.y)), DragMode.ScaleVertical, false);
                ...
                m_SelectionScaleTop.onDrag += (TimelineWindowManipulator manipulator, Event evt) =>
                {
                    OnScaleValue(PixelToValue(evt.mousePosition.y));
                    return true;
                };
                m_SelectionScaleTop.onEndDrag += (TimelineWindowManipulator manipulator, Event evt) =>
                {
                    OnEndScale();
                    return true;
                };
        ...
        public void OnGUI()
        {
            ...
            TimelineEditor.CurveEditorSettings.RectangleToolFlags flags = m_CurveEditor.settings.rectangleToolFlags;
            if (flags == TimelineEditor.CurveEditorSettings.RectangleToolFlags.NoRectangleTool)
                return;
            ...
            if (flags == TimelineEditor.CurveEditorSettings.RectangleToolFlags.FullRectangleTool)
            {
        ...
        private void DrawLabels(Rect bounds)
        {
            ...
            else if (flags == TimelineEditor.CurveEditorSettings.RectangleToolFlags.MiniRectangleTool)
            {
