    ...
    internal class DopeSheetEditor : TimeArea, CurveUpdater
    {
        ...
        public float contentHeight
        {
            get
            {
                ...
                foreach (DopeLine dopeline in state.dopelines)
                    height += dopeline.tallMode ? TimelineWindowHierarchyGUI.k_DopeSheetRowHeightTall : TimelineWindowHierarchyGUI.k_DopeSheetRowHeight;

                height += TimelineWindowHierarchyGUI.k_AddCurveButtonNodeHeight;
                return height;
            }
        }
        ...
        private Rect DopelinesGUI(Rect position, Vector2 scrollPosition)
        {
            ...
            for (int i = 0; i < dopelines.Count; ++i)
            {
                ...
                dopeLine.position.height = (dopeLine.tallMode ? TimelineWindowHierarchyGUI.k_DopeSheetRowHeightTall : TimelineWindowHierarchyGUI.k_DopeSheetRowHeight);
        ...
        private void DopeLineRepaint(DopeLine dopeline)
        {
            ...
            TimelineWindowHierarchyNode node = (TimelineWindowHierarchyNode)state.hierarchyData.FindItem(dopeline.hierarchyNodeID);
            ...
            {
                TimelineWindowKeyframe keyframe = dopeline.keys[i];
                // Hash optimizations
            ...
            {
                float time = Mathf.Max(state.PixelToTime(Event.current.mousePosition.x, TimelineWindowState.SnapMode.SnapToFrame), 0f);
        ...
        private GenericMenu GenerateMenu(DopeLine dopeline)
        {
            ...
            List<TimelineWindowKeyframe> hoveringKeys = new List<TimelineWindowKeyframe>();
            ...
            TimelineKeyTime mouseKeyTime = TimelineKeyTime.Time(state.PixelToTime(Event.current.mousePosition.x, TimelineWindowState.SnapMode.SnapToFrame), state.frameRate);
            ...
            if (dopeline.isEditable && TimelineWindowUtility.ContainsFloatKeyframes(state.selectedKeys))
            {
                menu.AddSeparator(string.Empty);

                List<KeyIdentifier> keyList = new List<KeyIdentifier>();
                Hashtable editorCurves = new Hashtable();
                foreach (TimelineWindowKeyframe key in state.selectedKeys)
                {
                    if (key.isDiscreteCurve)
                        continue;

                    int index = key.curve.GetKeyframeIndex(TimelineKeyTime.Time(key.time, state.frameRate));
                    if (index == -1)
                        continue;
        ...
        private void HandleDragging()
        {
            ...
            if ((eventType == EventType.MouseDrag || eventType == EventType.MouseUp) && m_MousedownOnKeyframe)
            {
                ...
                foreach (TimelineWindowKeyframe selectedKey in state.selectedKeys)
                    firstSelectedKeyTime = Mathf.Min(selectedKey.time, firstSelectedKeyTime);

                float currentTime = state.SnapToFrame(state.PixelToTime(Event.current.mousePosition.x), TimelineWindowState.SnapMode.SnapToFrame);
        ...
        private void DoSpriteDropAfterGeneratingNewDopeline(AnimationClip animationClip, EditorCurveBinding? spriteBinding)
        {
            ...
            TimelineWindowCurve newCurve = new TimelineWindowCurve(animationClip, (EditorCurveBinding)spriteBinding, typeof(Sprite));
        ...
        private void AssignSpriteToSpriteRenderer(TimelineWindowCurve curve)
        {
        ...
        private EditorCurveBinding? CreateNewPptrDopeline(TimelineWindowSelectionItem selectedItem, Type valueType)
        {
            List<EditorCurveBinding> potentialBindings = null;
            if (selectedItem.rootGameObject != null)
            {
                potentialBindings = TimelineWindowUtility.GetAnimatableProperties(selectedItem.rootGameObject, selectedItem.rootGameObject, valueType);
            ...
            {
                potentialBindings = TimelineWindowUtility.GetAnimatableProperties(selectedItem.scriptableObject, valueType);
            }
        ...
        private EditorCurveBinding? CreateNewSpriteRendererDopeline(GameObject targetGameObject, GameObject rootGameObject)
        {
            ...
            List<EditorCurveBinding> curveBindings = TimelineWindowUtility.GetAnimatableProperties(targetGameObject, rootGameObject, typeof(SpriteRenderer), typeof(Sprite));
        ...
        private void HandleMouseDown(DopeLine dopeline)
        {
            ...
            {
                foreach (TimelineWindowKeyframe key in dopeline.keys)
                {
        ...
        {
            float timeAtMousePosition = state.PixelToTime(Event.current.mousePosition.x, TimelineWindowState.SnapMode.SnapToFrame);
            TimelineKeyTime mouseKeyTime = TimelineKeyTime.Time(timeAtMousePosition, state.frameRate);
            TimelineWindowUtility.AddKeyframes(state, dopeline.curves, mouseKeyTime);
        ...
        private Rect GetKeyframeRect(DopeLine dopeline, TimelineWindowKeyframe keyframe)
        {
            ...
            return new Rect(state.TimeToPixel(state.SnapToFrame(time, TimelineWindowState.SnapMode.SnapToFrame)) + GetKeyframeOffset(dopeline, keyframe), dopeline.position.yMin, width, dopeline.position.height);
        }
        ...
        private float GetKeyframeOffset(DopeLine dopeline, TimelineWindowKeyframe keyframe)
        {
            if (dopeline.isPptrDopeline && dopeline.tallMode && (keyframe == null || keyframe.value != null))
        ...
        public void FrameSelected()
        {
            ...
            if (!keyframesSelected)
            {
                ...
                {
                    foreach (TimelineWindowCurve curve in state.activeCurves)
                    {
        ...
        private bool DoDragAndDrop(DopeLine dopeLine, Rect position, bool perform)
        {
            ...
            TimelineWindowCurve curve = null;
            ...
            else
            {
                ...
                foreach (TimelineWindowCurve dopelineCurve in dopeLine.curves)
                {
            ...
            {
                float time = Mathf.Max(state.PixelToTime(Event.current.mousePosition.x, TimelineWindowState.SnapMode.SnapToFrame), 0f);
                TimelineWindowCurve targetCurve = GetCurveOfType(dopeLine, targetType);
        ...
        private void PerformDragAndDrop(TimelineWindowCurve targetCurve, float time)
        {
            ...
            int startFrame = TimelineKeyTime.Time(time, targetCurve.clip.frameRate).frame;
            ...
            for (int i = 0; i < objectReferences.Length; ++i)
            {
                ...
                CreateNewPPtrKeyframe(TimelineKeyTime.Frame(startFrame + i, targetCurve.clip.frameRate).time, value, targetCurve);
            }
        ...
        private void CreateNewPPtrKeyframe(float time, Object value, TimelineWindowCurve targetCurve)
        {
            ...
            TimelineWindowKeyframe keyframe = new TimelineWindowKeyframe(targetCurve, referenceKeyframe);
            TimelineKeyTime newTime = TimelineKeyTime.Time(keyframe.time, state.frameRate);
        ...
        private TimelineWindowCurve GetCurveOfType(DopeLine dopeLine, System.Type type)
        {
            foreach (TimelineWindowCurve curve in dopeLine.curves)
            {
        ...
        {
            TimelineWindowKeyframe keyframe = dopeLine.keys[keyIndex];
        ...
        {
            TimelineWindowUtility.AddKeyframes(state, context.dopeline.curves, context.time);
        }

        private void DeleteKeys(object obj) { DeleteKeys((List<TimelineWindowKeyframe>)obj); }
        private void DeleteKeys(List<TimelineWindowKeyframe> keys)
        {
        ...
        internal class DopeSheetSelectionRect
        {
                    ...
                    case EventType.MouseUp:
                        if (GUIUtility.hotControl == id && evt.button == 0)
                        {
                            if (m_ValidRect)
                            {
                                ...
                                HashSet<TimelineWindowKeyframe> toBeUnselected = new HashSet<TimelineWindowKeyframe>();
                                HashSet<TimelineWindowKeyframe> toBeSelected = new HashSet<TimelineWindowKeyframe>();
                                ...
                                foreach (DopeLine dopeline in owner.state.dopelines)
                                {
                                    if (dopeline.position.yMin >= timeRect.yMin && dopeline.position.yMax <= timeRect.yMax)
                                    {
                                        foreach (TimelineWindowKeyframe keyframe in dopeline.keys)
                                        {
                                            TimelineKeyTime startTime = TimelineKeyTime.Time(timeRect.xMin, frameRate);
                                            TimelineKeyTime endTime = TimelineKeyTime.Time(timeRect.xMax, frameRate);
                                ...
                                if (toBeSelected.Count == 0)
                                    foreach (TimelineWindowKeyframe keyframe in toBeUnselected)
                                        owner.state.UnselectKey(keyframe);

                                foreach (TimelineWindowKeyframe keyframe in toBeSelected)
            ...
            {
                float height = TimelineWindowHierarchyGUI.k_DopeSheetRowHeight;
                Rect r = TimelineWindowUtility.FromToRect(m_SelectStartPoint, m_SelectMousePoint);
                r.xMin = owner.state.TimeToPixel(owner.state.PixelToTime(r.xMin, TimelineWindowState.SnapMode.SnapToFrame), TimelineWindowState.SnapMode.SnapToFrame);
                r.xMax = owner.state.TimeToPixel(owner.state.PixelToTime(r.xMax, TimelineWindowState.SnapMode.SnapToFrame), TimelineWindowState.SnapMode.SnapToFrame);
            ...
            public Rect GetCurrentTimeRect()
            {
                float height = TimelineWindowHierarchyGUI.k_DopeSheetRowHeight;
                Rect r = TimelineWindowUtility.FromToRect(m_SelectStartPoint, m_SelectMousePoint);
                r.xMin = owner.state.PixelToTime(r.xMin, TimelineWindowState.SnapMode.SnapToFrame);
                r.xMax = owner.state.PixelToTime(r.xMax, TimelineWindowState.SnapMode.SnapToFrame);
                r.yMin = Mathf.Floor(r.yMin / height) * height;
                r.yMax = (Mathf.Floor(r.yMax / height) + 1) * height;
                return r;
            }
        ...
        public void UpdateCurves(List<ChangedCurve> changedCurves, string undoText)
        {
            ...
            {
                TimelineWindowCurve curve = state.allCurves.Find(c => changedCurve.curveId == c.GetHashCode());
                if (curve != null)
                {
