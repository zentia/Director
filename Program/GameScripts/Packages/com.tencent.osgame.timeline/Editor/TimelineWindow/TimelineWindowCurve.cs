...
namespace TimelineEditorInternal
{
    internal class TimelineWindowCurve : IComparable<TimelineWindowCurve>, IEquatable<TimelineWindowCurve>
    {
        ...
        private TimelineWindowSelectionItem m_SelectionBinding;
        ...
        public TimelineWindowSelectionItem selectionBinding { get { return m_SelectionBinding; } set { m_SelectionBinding = value; } }

        public TimelineWindowCurve(AnimationClip clip, EditorCurveBinding binding, System.Type valueType)
        {
        ...
        public void LoadKeyframes(AnimationCurve curve)
        {
            ...
            for (int i = 0; i < curve.length; i++)
                m_Keyframes.Add(new TimelineWindowKeyframe(this, curve[i]));
        }

        public void LoadKeyframes(AnimationClip clip)
        {
            m_Keyframes = new List<TimelineWindowKeyframe>();
        ...
        public AnimationCurve ToAnimationCurve()
        {
            ...
            for (int i = 0; i < length; i++)
            {
                ...
                if (Mathf.Abs(m_Keyframes[i].time - lastFrameTime) > TimelineWindowCurve.timeEpsilon)
                {
        ...
        public TimelineWindowKeyframe FindKeyAtTime(TimelineKeyTime keyTime)
        {
        ...
        public object Evaluate(float time)
        {
            ...
            TimelineWindowKeyframe firstKey = m_Keyframes[0];
            if (time <= firstKey.time)
                return firstKey.value;

            TimelineWindowKeyframe lastKey = m_Keyframes[m_Keyframes.Count - 1];
            if (time >= lastKey.time)
                return lastKey.value;

            TimelineWindowKeyframe key = firstKey;
            for (int i = 1; i < m_Keyframes.Count; ++i)
            {
                TimelineWindowKeyframe nextKey = m_Keyframes[i];
        ...
        public void AddKeyframe(TimelineWindowKeyframe key, TimelineKeyTime keyTime)
        {
        ...
        public void RemoveKeyframe(TimelineKeyTime time)
        {
        ...
        public bool HasKeyframe(TimelineKeyTime time)
        {
            return GetKeyframeIndex(time) != -1;
        }

        public int GetKeyframeIndex(TimelineKeyTime time)
        {
