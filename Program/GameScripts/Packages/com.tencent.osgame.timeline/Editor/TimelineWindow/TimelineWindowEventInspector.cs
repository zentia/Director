...
{
    [CustomEditor(typeof(TimelineWindowEvent))]
    ...
    internal class AnimationWindowEventInspector : Editor
    {
        ...
        TimelineEventEditorState m_State = new();

        public override void OnInspectorGUI()
        {
            var awes = targets.Select(o => o as TimelineWindowEvent).ToArray();
        ...
        public static void OnEditAnimationEvent(TimelineWindowEvent awe, TimelineEventEditorState state)
        {
            OnEditAnimationEvents(new TimelineWindowEvent[] {awe}, state);
        }
        ...
        public static void OnEditAnimationEvents(TimelineWindowEvent[] awEvents, TimelineEventEditorState state)
        {
                ...
                while (type != typeof(MonoBehaviour) && type != null)
                {
                    if (!s_TypeAnimationMethodMapCache.TryGetValue(type, out IReadOnlyList<AnimationMethodMap> validMethods))
                    {
                        ...
                        for (int i = 0; i < methods.Length; i++)
                        {
                            ...
                            if (null == method.GetCustomAttribute<AnimationEventNameAttribute>())
                                continue;
        ...
        private static AnimationWindowEventData GetData(TimelineWindowEvent[] awEvents)
        {
            var data = new AnimationWindowEventData();
            if (awEvents.Length == 0)
                return data;

            TimelineWindowEvent firstAwEvent = awEvents[0];
        ...
        private static void SetData(TimelineWindowEvent[] awEvents, AnimationWindowEventData data)
        {
            if (data.events == null)
                return;
        ...
        [MenuItem("CONTEXT/TimelineWindowEvent/Reset", secondaryPriority = 7)]
        static void ResetValues(MenuCommand command)
        {
            TimelineWindowEvent awEvent = command.context as TimelineWindowEvent;
            TimelineWindowEvent[] awEvents = new TimelineWindowEvent[] { awEvent };
