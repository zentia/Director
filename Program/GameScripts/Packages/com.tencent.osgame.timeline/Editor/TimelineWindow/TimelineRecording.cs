...
namespace TimelineEditorInternal
{
    internal class AnimationRecording
    {
        ...
        static private UndoPropertyModification[] FilterModifications(ITimelineRecordingState state, ref UndoPropertyModification[] modifications)
        {
        ...
        static private void CollectRotationModifications(ITimelineRecordingState state, ref UndoPropertyModification[] modifications,
        ...
        static private UndoPropertyModification[] FilterRotationModifications(ITimelineRecordingState state, ref Dictionary<object, RotationModification> rotationModifications)
        {
            GameObject rootGameObject = state.activeRootGameObject;
        ...
        static private void AddRotationPropertyModification(ITimelineRecordingState state, EditorCurveBinding baseBinding, UndoPropertyModification modification)
        {
        ...
        static private void ProcessRotationModifications(ITimelineRecordingState state, ref Dictionary<object, RotationModification> rotationModifications)
        {
            foreach (KeyValuePair<object, RotationModification> item in rotationModifications)
            {
                RotationModification m = item.Value;
        ...
        static private void CollectVector3Modifications(ITimelineRecordingState state, ref UndoPropertyModification[] modifications,
        ...
        static private void ProcessVector3Modification(ITimelineRecordingState state, EditorCurveBinding baseBinding, UndoPropertyModification modification, Transform target, string axis, float scale = 1.0f)
        {
        ...
        static public void ProcessVector3Modifications(ITimelineRecordingState state, ref Dictionary<object, Vector3Modification> vector3Modifications)
        {
        ...
        static public void ProcessModifications(ITimelineRecordingState state, UndoPropertyModification[] modifications)
        {
        ...
        static public UndoPropertyModification[] Process(ITimelineRecordingState state, UndoPropertyModification[] modifications)
        {
            GameObject root = state.activeRootGameObject;
            if (root == null)
        ...
        static void AddKey(ITimelineRecordingState state, EditorCurveBinding binding, Type type, object previousValue, object currentValue)
        {
            ...
            TimelineWindowCurve curve = new TimelineWindowCurve(clip, binding, type);
            ...
            if (state.addZeroFrame)
            {
                ...
                if (curve.length == 0)
                {
                    ...
                    {
                        TimelineWindowUtility.AddKeyframeToCurve(curve, previousValue, type, TimelineKeyTime.Frame(0, clip.frameRate));
                    }
                }
            }

            // Add key at current frame.
            TimelineWindowUtility.AddKeyframeToCurve(curve, currentValue, type, TimelineKeyTime.Frame(state.currentFrame, clip.frameRate));

            state.SaveCurve(curve);
        }

        static void AddRotationKey(ITimelineRecordingState state, EditorCurveBinding binding, Type type, Vector3 previousEulerAngles, Vector3 currentEulerAngles)
        {
            AnimationClip clip = state.activeAnimationClip;
            ...
            for (int i = 0; i < 3; i++)
            {
                TimelineWindowCurve curve = new TimelineWindowCurve(clip, additionalBindings[i], type);
                ...
                if (state.addZeroFrame)
                {
                    ...
                    if (curve.length == 0)
                    {
                        if (state.currentFrame != 0)
                        {
                            TimelineWindowUtility.AddKeyframeToCurve(curve, previousEulerAngles[i], type, TimelineKeyTime.Frame(0, clip.frameRate));
                        }
                    }
                }

                TimelineWindowUtility.AddKeyframeToCurve(curve, currentEulerAngles[i], type, TimelineKeyTime.Frame(state.currentFrame, clip.frameRate));
                state.SaveCurve(curve);
            }
        }
        ...
        static private void ProcessRootMotionModifications(ITimelineRecordingState state,
        ...
        static private void ProcessAnimatorModifications(ITimelineRecordingState state,
        ...
        static void ProcessRootMotionModification(ITimelineRecordingState state, Animator animator, UndoPropertyModification modification, string name, float value, float scale)
        {
            AnimationClip clip = state.activeAnimationClip;
            ...
            TimelineWindowCurve curve = new TimelineWindowCurve(clip, binding, typeof(float));

            if (state.addZeroFrame && state.currentFrame != 0 && curve.length == 0)
            {
                TimelineWindowUtility.AddKeyframeToCurve(curve, prevValue, typeof(float), TimelineKeyTime.Frame(0, clip.frameRate));
            }

            TimelineWindowUtility.AddKeyframeToCurve(curve, value, typeof(float), TimelineKeyTime.Frame(state.currentFrame, clip.frameRate));
