...
namespace TimelineEditor
{
    ...
    internal class TimeEditor : ScriptableObject
    {
        ...
        [SerializeField] private TimelineWindowState m_State;
        [SerializeField] private TimelineEditorInternal.DopeSheetEditor m_DopeSheet;
        [SerializeField] private TimelineWindowHierarchy m_Hierarchy;
        [SerializeField] private TimelineWindowClipPopup m_ClipPopup;
        [SerializeField] private TimelineEventTimeLine m_Events;
        [SerializeField] private CurveEditor m_CurveEditor;
        [SerializeField] private TimelineEditorInternal.AnimEditorOverlay m_Overlay;
        ...
        public TimelineWindowState state { get { return m_State; } }
        ...
        internal TimelineEditorInternal.DopeSheetEditor dopeSheetEditor { get { return m_DopeSheet; } }
        ...
        public void OnEnable()
        {
            ...
            if (m_State == null)
            {
                m_State = CreateInstance(typeof(TimelineWindowState)) as TimelineWindowState;
            ...
            m_State.timeArea = m_State.showCurveEditor ? (UnityEditor.TimeArea)m_CurveEditor : m_DopeSheet;
        ...
        private GenericMenu GenerateOptionsMenu()
        {
            ...
            menu.AddItem(EditorGUIUtility.TextContent("Seconds"), m_State.timeFormat == UnityEditor.TimeArea.TimeFormat.TimeFrame, () => m_State.timeFormat = UnityEditor.TimeArea.TimeFormat.TimeFrame);
            menu.AddItem(EditorGUIUtility.TextContent("Frames"), m_State.timeFormat == UnityEditor.TimeArea.TimeFormat.Frame, () => m_State.timeFormat = UnityEditor.TimeArea.TimeFormat.Frame);
        ...
        private void AddKeyframeButtonOnGUI()
        {
            ...
            using (new EditorGUI.DisabledScope(!canAddKey))
            {
                if (GUILayout.Button(AnimationWindowStyles.addKeyframeContent, AnimationWindowStyles.animClipToolbarButton))
                {
                    ...
                    TimelineWindowUtility.AddSelectedKeyframes(m_State, keyTime);
                    UpdateSelectedKeysToCurveEditor();
        ...
        static void ExecuteShortcut(ShortcutArguments args, Action<TimeEditor> exp)
        {
            var animationWindow = (NewTimelineWindow)args.context;
        ...
        static void ExecuteShortcut(ShortcutArguments args, Action<ITimelineWindowControl> exp)
        {
        ...
        static void KeySelected(ShortcutArguments args)
        {
            ...
            TimelineWindowUtility.AddSelectedKeyframes(animEditor.m_State, animEditor.controlInterface.time);
        ...
        private void UpdateSelectedKeysFromCurveEditor()
        {
            ...
            {
                TimelineWindowKeyframe keyFrame = TimelineWindowUtility.CurveSelectionToAnimationWindowKeyframe(curveSelection, m_State.allCurves);
                if (keyFrame != null)
        ...
        private void UpdateSelectedKeysToCurveEditor()
        {
            ...
            foreach (TimelineWindowKeyframe keyframe in m_State.selectedKeys)
            {
                TimelineEditor.CurveSelection curveSelection = TimelineWindowUtility.AnimationWindowKeyframeToCurveSelection(keyframe, m_CurveEditor);
        ...
        {
            m_ClipPopup = new TimelineWindowClipPopup();
        }
        ...
        {
            // The rect here is only for initialization and will be overriden at layout
            m_Hierarchy = new TimelineWindowHierarchy(m_State, m_OwnerWindow, new Rect(0, 0, hierarchyWidth, 100));
        }

        // Called once during initialization of m_State
        private void InitializeDopeSheet()
        {
            m_DopeSheet = new TimelineEditorInternal.DopeSheetEditor(m_OwnerWindow);
            m_DopeSheet.SetTickMarkerRanges();
        ...
        {
            m_Events = new TimelineEventTimeLine(m_OwnerWindow);
        }
        ...
        {
            m_Overlay = new TimelineEditorInternal.AnimEditorOverlay();
        }
