...
namespace CinemaDirector
{
    ...
    public abstract class CinemaGlobalAction : TimelineAction, IComparable
    {
        ...
        public int CompareTo(object other)
        {
            ...
            return (int)(otherAction.Firetime - Firetime);
        }

        public GameObject GetActor(string targetPath)
        {
            var target = Cutscene.transform.Find(targetPath);
            if (!target)
                return null;
            var actorCtrl = target.GetComponent<ActorTrackGroup>();
            if (actorCtrl == null || actorCtrl.Actor == null)
                return null;
            return actorCtrl.Actor.gameObject;
        }
    }
}