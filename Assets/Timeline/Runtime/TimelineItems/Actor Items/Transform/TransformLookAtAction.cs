using System.Collections.Generic;
using UnityEngine;

namespace TimelineRuntime
{
    /// <summary>
    /// An action that updates the transform of the Actor to look at a target game object.
    /// </summary>
    [TimelineItem("Transform", "Look At", TimelineItemGenre.ActorItem, TimelineItemGenre.TransformItem)]
    public class TransformLookAtAction : TimelineActorAction, IRecoverableObject
    {
        [SerializeField]
        [Tooltip("The target that the Actor should look at.")]
        public string LookAtTarget;

        private Transform m_LookAtTarget;

        // Options for reverting during runtime.
        [SerializeField]
        private RevertMode runtimeRevertMode = RevertMode.Revert;


        /// <summary>
        /// Cache the state of all actors related to this event.
        /// </summary>
        /// <returns>Info to revert rotation</returns>
        public RevertInfo[] CacheState()
        {
            var actors = (GetActor());
            List<RevertInfo> reverts = new List<RevertInfo>();
            {
                Transform go = actors;
                if (go != null)
                {
                    Transform t = go.GetComponent<Transform>();
                    if (t != null)
                    {
                        reverts.Add(new RevertInfo(this, t, "localRotation", t.localRotation));
                    }
                }
            }

            return reverts.ToArray();
        }

        /// <summary>
        /// Trigger this action and have the actor look at the target.
        /// </summary>
        /// <param name="actor">The actor to update the transform of.</param>
        public override void Trigger(GameObject actor)
        {
            if (actor == null || string.IsNullOrEmpty(LookAtTarget)) return;
            m_LookAtTarget = timeline.GetActor(LookAtTarget);
            if (m_LookAtTarget == null)
            {
                return;
            }
            LookAt(actor);
        }

        /// <summary>
        /// Continue to update the transform to look at the target.
        /// </summary>
        /// <param name="actor">The actor being updated.</param>
        /// <param name="runningTime">The running time of the timeline.</param>
        /// <param name="deltaTime">The deltaTime since last call.</param>
        public override void UpdateTime(GameObject actor, float runningTime, float deltaTime)
        {
            if (actor == null || m_LookAtTarget == null) return;
            LookAt(actor);
        }

        private void LookAt(GameObject actor)
        {
            actor.transform.LookAt(m_LookAtTarget);
            var localEulerAngles = actor.transform.localEulerAngles;
            localEulerAngles.x = 0;
            localEulerAngles.z = 0;
            actor.transform.localEulerAngles = localEulerAngles;
        }

        /// <summary>
        /// End the action.
        /// </summary>
        /// <param name="actor">The actor of this action.</param>
        public override void End(GameObject actor)
        {
            // Do nothing.
        }

        /// <summary>
        /// Option for choosing when this Event will Revert to initial state in Runtime.
        /// </summary>
        public RevertMode RuntimeRevertMode
        {
            get { return runtimeRevertMode; }
            set { runtimeRevertMode = value; }
        }
    }
}
