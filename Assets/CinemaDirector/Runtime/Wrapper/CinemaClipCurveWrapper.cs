...
{
	public class CinemaClipCurveWrapper : TimelineItemWrapper
	{
		...
		public CinemaClipCurveWrapper(TimelineItem behaviour, float firetime, float duration) : base(behaviour, firetime)
		{
		}
		...
		public float Duration
        {
			get
            {
				return TimelineItem.Cutscene.Duration;
			}
        }

		public void CropFiretime(float newFiretime)
		{
		...
		public void ScaleFiretime(float newFiretime)
		{
			if (newFiretime >= base.Firetime + Duration)
			{
				return;
			}
			...
			for (int i = 0; i < memberCurves.Length; i++)
			{
				...
				for (int j = 0; j < animationCurves.Length; j++)
				{
					animationCurves[j].ScaleStart(base.Firetime, Duration, newFiretime);
				}
			}
			base.Firetime = newFiretime;
		}

		public void CropDuration(float newDuration)
		{
			if (newDuration <= 0f)
			{
				return;
			...
			{
				this.updateKeyframeTime(base.Firetime + Duration, base.Firetime + newDuration);
			}
			else
			{
				CinemaMemberCurveWrapper[] memberCurves = this.MemberCurves;
				for (int i = 0; i < memberCurves.Length; i++)
				{
					...
					for (int j = 0; j < animationCurves.Length; j++)
					{
						animationCurves[j].CollapseEnd(base.Firetime + Duration, base.Firetime + newDuration);
					}
				}
			}
		}

		public void ScaleDuration(float newDuration)
		{
			if (newDuration <= 0f)
			{
				return;
