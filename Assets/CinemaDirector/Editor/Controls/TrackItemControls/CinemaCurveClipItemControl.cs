...
{
	public abstract class CinemaCurveClipItemControl : TrackItemControl
	{
		...
		public override void PreUpdate(DirectorControlState state, Rect trackPosition)
		{
            ...
            {
				base.PreUpdate(state, trackPosition);
				return;
            }
			var cinemaClipCurveWrapper = Wrapper as CinemaClipCurveWrapper;
			...
			{
				timelineItemPosition = new Rect(controlPosition.x, controlPosition.y, controlPosition.width, 17f);
			}
			if (isEditing)
			{
				masterKeysPosition = new Rect(controlPosition.x, timelineItemPosition.y + timelineItemPosition.height, this.controlPosition.width, 17f);
		...
		private void UpdateKeyframes(CinemaClipCurveWrapper clipWrapper, DirectorControlState state)
		{
			...
			{
				var cinemaMemberCurveWrapper = clipWrapper.MemberCurves[i];
				for (int j = 0; j < cinemaMemberCurveWrapper.AnimationCurves.Length; j++)
				{
					var cinemaAnimationCurveWrapper = cinemaMemberCurveWrapper.AnimationCurves[j];
					for (int k = 0; k < cinemaAnimationCurveWrapper.KeyframeCount; k++)
					{
						Keyframe keyframe = cinemaAnimationCurveWrapper.GetKeyframe(k);
						var screenPosition = new Vector2(state.TimeToPosition(keyframe.time), num2 - (keyframe.value - viewingSpace.y) / num * curveTrackSafeArea.height);
						var keyframeWrapper = cinemaAnimationCurveWrapper.GetKeyframeWrapper(k);
		...
		private void UpdateKeyframe(DirectorControlState state)
        {
			var num = viewingSpace.height - viewingSpace.y;
			var item = Behaviour as TimelineItem;
			var track = item.Parent as CurveTrack;
		...
		public override void HandleInput(DirectorControlState state, Rect trackPosition)
		{
			if (!state.IsInCurveMode)
            {
				base.HandleInput(state, trackPosition);
				return;
            }
			var cinemaClipCurveWrapper = Wrapper as CinemaClipCurveWrapper;
			if (cinemaClipCurveWrapper == null)
						...
						for (int k = 0; k < cinemaAnimationCurveWrapper.KeyframeCount; k++)
						{
							...
							case EventType.MouseDown:
								if (rect.Contains(Event.current.mousePosition))
								{
									...
									DirectorWindow.GetSelection().activeObject = Wrapper.TimelineItem;
		...
		private void handleMasterKeysInput(CinemaClipCurveWrapper clipCurveWrapper, DirectorControlState state)
		{
			...
			for (int i = 0; i < keyframeTimes.Count; i++)
			{
				...
				case EventType.MouseDrag:
					...
					{
						Undo.RecordObject(Wrapper.TimelineItem, "Moved Keyframes");
		...
		private void handleItemInput(CinemaClipCurveWrapper clipCurveWrapper, DirectorControlState state, Rect trackPosition)
		{
			...
			this.controlID = GUIUtility.GetControlID(Wrapper.TimelineItem.GetInstanceID(), FocusType.Passive, timelineItemPosition);
			int controlID = GUIUtility.GetControlID(Wrapper.TimelineItem.GetInstanceID(), (FocusType)2, rect);
			int controlID2 = GUIUtility.GetControlID(Wrapper.TimelineItem.GetInstanceID(), (FocusType)2, rect2);
			int controlID3 = GUIUtility.GetControlID(Wrapper.TimelineItem.GetInstanceID(), (FocusType)2, rect3);
			...
			case EventType.MouseDown:
				if (rect2.Contains(Event.current.mousePosition) && Event.current.button == 0)
				{
					...
					if (Event.current.control)
					{
						if (base.IsSelected)
						{
							DirectorWindow.GetSelection().Remove(base.Wrapper.TimelineItem);
							this.hasSelectionChanged = true;
						}
						else
						{
							DirectorWindow.GetSelection().Add(base.Wrapper.TimelineItem);
							this.hasSelectionChanged = true;
						}
					}
				...
				if (GUIUtility.hotControl == controlID)
				{
					...
					using (IEnumerator<TimelineItemWrapper> enumerator = base.Track.Items.GetEnumerator())
					{
						while (enumerator.MoveNext())
						{
							...
							if (cinemaActionWrapper != null && cinemaActionWrapper.TimelineItem != base.Wrapper.TimelineItem)
							{
				...
				if (GUIUtility.hotControl == controlID3)
				{
					...
					using (IEnumerator<TimelineItemWrapper> enumerator = base.Track.Items.GetEnumerator())
					{
						while (enumerator.MoveNext())
						{
							...
							if (cinemaActionWrapper2 != null && cinemaActionWrapper2.TimelineItem != base.Wrapper.TimelineItem)
							{
			...
			if (Event.current.type == EventType.KeyDown && Event.current.keyCode == (KeyCode)127 && DirectorWindow.GetSelection().activeObject == base.Wrapper.TimelineItem)
			{
		...
		internal override void ConfirmTranslate()
		{
			...
			{
				this.TranslateCurveClipItem(this, new CurveClipItemEventArgs(cinemaClipCurveWrapper.TimelineItem, cinemaClipCurveWrapper.Firetime, cinemaClipCurveWrapper.Duration));
			}
			haveCurvesChanged = true;
		}
		...
		protected void drawCurveItem()
		{
			...
			rect.width= 10;
			rect.height= 10;
			GUI.Box(rect, actionIcon, GUIStyle.none);
			GUI.color = temp;
		...
		private void snapScrubber(object userData)
		{
			...
			{
				SnapScrubber(this, new CurveClipScrubberEventArgs(Wrapper.TimelineItem, curvesContext.time));
		...
		internal void UpdateHeaderArea(DirectorControlState state, Rect controlHeaderArea)
		{
			...
			for (int i = 0; i < memberCurves.Length; i++)
			{
				...
				int controlID = GUIUtility.GetControlID(Wrapper.TimelineItem.GetInstanceID(), FocusType.Passive, rect4);
				num2++;
				if (cinemaMemberCurveWrapper.IsFoldedOut)
				{
					...
					for (int j = 0; j < animationCurves.Length; j++)
					{
						...
						int controlID2 = GUIUtility.GetControlID(Wrapper.TimelineItem.GetInstanceID(), (FocusType)2, rect7);
