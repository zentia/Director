...
{
    [CutsceneItemControl(typeof(CinemaClipCurve))]
    public class CinemaCurveControl : CinemaCurveClipItemControl
    {
        ...
        {
            TranslateCurveClipItem += CinemaCurveControl_TranslateCurveClipItem;
            SnapScrubber += CinemaCurveControl_SnapScrubber;
            CurvesChanged += CinemaCurveControl_CurvesChanged;
            actionIcon = Resources.Load<Texture>("Director_SolidKeyframe");
        }
        ...
        protected virtual void CinemaCurveControl_CurvesChanged(object sender, CurveClipWrapperEventArgs e)
        {
            ...
            CinemaClipCurve clipCurve = wrapper.TimelineItem as CinemaClipCurve;
            ...
            for (int i = 0; i < clipCurve.CurveData.Count; i++)
            {
                ...
                CinemaMemberCurveWrapper memberWrapper;
        ...
        {
            CinemaClipCurve clipCurve = clipWrapper.TimelineItem as CinemaClipCurve;
            if (clipCurve == null) 
                return;
            ...
            for (int i = 0; i < clipCurve.CurveData.Count; i++)
            {
                ...
                if (!clipWrapper.TryGetValue(member.Type, member.PropertyName, out memberWrapper))
                {
                    ...
                    ArrayUtility.Add(ref clipWrapper.MemberCurves, memberWrapper);
            ...
            {
                ArrayUtility.Remove(ref clipWrapper.MemberCurves, item);
            }
        }
    }
}