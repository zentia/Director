...
public class CinemaActorCurveControl : CinemaCurveControl
{
    ...
    {
        CinemaActorClipCurve clipCurve = Wrapper.TimelineItem as CinemaActorClipCurve;
    ...
    protected override void ShowContextMenu(DirectorObject behaviour)
    {
        ...
        {
            createMenu.AddSeparator(string.Empty);
            var component = clipCurve.Parent;
            ...
            for (int i = 0; i < members.Length; i++)
            {
                ...
                context.component = clipCurve.Parent;
                context.memberInfo = members[i];
                if (!currentCurves.Contains(new KeyValuePair<string, string>(component.GetType().Name, members[i].Name)))
                {
                    createMenu.AddItem(new GUIContent(string.Format("Add Curve/{0}/{1}", component.GetType().Name, DirectorHelper.GetUserFriendlyName(component, members[i]))), false, addCurve, context);
                }
            }
    ...
    private class AddCurveContext
    {
        ...
        public DirectorObject component;
