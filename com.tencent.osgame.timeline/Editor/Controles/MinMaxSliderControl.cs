    ...
    internal class MinMaxSliderControl
    {
        ...
        internal static void DoMinMaxSlider(Rect position, int id, ref float value, ref float size, float visualStart, float visualEnd, float startLimit, float endLimit, GUIStyle slider, GUIStyle thumb, bool horiz)
        {
            ...
            if ((slider != null) && (thumb != null))
            {
                ...
                if (horiz)
                {
                    float num13 = (thumb.fixedWidth == 0f) ? thumb.padding.horizontal : thumb.fixedWidth;
                    ...
                    rect2 = new Rect(rect.x, rect.y, thumb.padding.left, rect.height);
                    rect3 = new Rect(rect.xMax - thumb.padding.right, rect.y, thumb.padding.right, rect.height);
                    num2 = current.mousePosition.x - position.x;
                }
                else
                {
                    float num14 = (thumb.fixedHeight == 0f) ? thumb.padding.vertical : thumb.fixedHeight;
                    ...
                    rect2 = new Rect(rect.x, rect.y, rect.width, thumb.padding.top);
                    rect3 = new Rect(rect.x, rect.yMax - thumb.padding.bottom, rect.width, thumb.padding.bottom);
